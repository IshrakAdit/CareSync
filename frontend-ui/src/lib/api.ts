...
class ApiClient {
  ...
  async addDoctorToHospital(hospitalId: string, data: {
    doctor_id: number;
  ...
  async requestNewDoctor(hospitalId: string, data: {
    name: string;
    specialty: string;
    phone_number: string;
    email: string;
    ...
    return this.request(`/hospital-admin/hospitals/${hospitalId}/doctors/request`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }

  async addTestToHospital(hospitalId: string, data: {
    test_id: number;
    cost: number;
    availability: string;
  }) {
  ...
  async requestNewTest(hospitalId: string, data: { name: string; description: string }) {
    return this.request(`/hospital-admin/hospitals/${hospitalId}/tests/request`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }

  async updateTest(hospitalId: string, testId: string, data: { cost: number; availability: string }) {
    return this.request(`/hospital-admin/hospitals/${hospitalId}/tests/${testId}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  }

  async addDepartmentToHospital(hospitalId: string, data: {
  ...
  async requestNewDepartment(hospitalId: string, data: { name: string; description: string }) {
    return this.request(`/hospital-admin/hospitals/${hospitalId}/departments/request`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
